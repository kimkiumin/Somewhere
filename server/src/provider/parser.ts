...
const VenueBaseSchema = z
  .object({
    ...
    dietaryTags: z.array(IdentifierSchema).max(20).default([]),
    allergenTags: z.array(IdentifierSchema).max(24).default([]),
    dietaryEvidence: z.enum(["reviewed", "unknown"]).default("unknown"),
    allergenEvidence: z.enum(["reviewed", "unknown"]).default("unknown"),
