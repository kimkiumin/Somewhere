...
export async function markJourneyStopped(
  ...
  await input.database
    .prepare(
      "UPDATE browser_session_guards SET guard_version = guard_version + 1, active_journey_digest = NULL, create_request_digest = NULL, previous_candidate_digest = ?, recovery_capability_digest = NULL, recovery_consumed_at = NULL, last_stopped_at = ? WHERE session_binding_digest = ? AND active_journey_digest = ?",
    )
    .bind(input.previousCandidateDigest, input.now, input.bindingDigest, input.journeyDigest)
    .run();
}
