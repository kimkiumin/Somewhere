...
(function initScreens(globalScope) {
  ...
  function renderJourney(state) {
    ...
    return `
      <section class="screen" aria-labelledby="journey-title">
        <div class="screen-copy">
          <p class="eyebrow">${state.phase === "arrived" ? "Arrival" : "Hidden destination"}</p>
          <h1 class="screen-heading" id="journey-title">${journeyHeading(state.phase)}</h1>
          <p class="screen-text">${escapeHtml(publicState.hint)}</p>
        </div>
        ${
          showCompass
        ...
        ${showCompass ? renderHiddenPanel(publicState, statusText(state)) : ""}
        <div class="actions">
          ${
