...
test("startAdventure selects a destination but keeps the public destination hidden", () => {
  ...
  assert.equal(publicState.routeBearingDeg, 24);
  assert.equal(publicState.userHeadingDeg, 6);
  assert.equal(publicState.directionLabel, "slight right");
...
test("moveCloser advances following to near and arrived thresholds", () => {
  ...
  assert.equal(near.routeBearingDeg, 40);
  assert.equal(near.userHeadingDeg, 7);
  assert.equal(arrived.phase, "arrived");
  assert.equal(arrived.distanceM, 20);
  assert.equal(app.toPublicJourney(arrived).directionLabel, "arrival zone");
});
...
test("renderApp uses a stripped-down compass without repeated journey panels", () => {
  const root = createRoot();
  const hidden = app.startAdventure(destinations, { index: 0, headingDeg: 24 });
  const following = app.beginFollowing(hidden);
  app.renderApp(root, { loading: false, state: following });

  assert.match(root.innerHTML, /compass-bearing-ring/);
  assert.equal(root.innerHTML.includes("route-signal-strip"), false);
  assert.equal(root.innerHTML.includes("direction-label"), false);
  assert.equal(root.innerHTML.includes("distance-unit"), false);
  assert.equal(root.innerHTML.includes("info-grid"), false);
  assert.equal(root.innerHTML.includes("Small Independent Bookstore"), false);
});
