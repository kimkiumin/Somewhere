...
현재 vNext 브라우저 프로토타입의 나침반 바늘을 유지하면서, 네비게이션처럼 다음 행동을 보조하는 하이브리드 안내로 확장한다. 목적지 이름·주소·건물 정보는 도착 전까지 계속 숨기지만, 이동에 필요한 방향과 거리는 제한하지 않는다.
...
나침반을 중심으로 다음 순서의 한 화면 레이아웃을 사용한다.
...
나침반 바늘이 방향을 담당하므로 `현재 방향` 텍스트는 중복 표시하지 않는다. 다음 행동은 화면 왼쪽 위에 작은 네비게이션 안내판처럼 배치하고, 전체 남은 거리는 나침반 아래에 둔다. 지도는 넣지 않는다. Stop은 기존처럼 안내를 즉시 일시정지시키며, 일시정지 중에는 행동 지시와 방향을 활성 상태로 표시하지 않는다.
...
- Following과 near에 나침반 바늘과 간결한 턴바이턴 정보가 함께 표시된다.
