...
| Date | Change | Notes |
|---|---|---|
| 2026-07-09 | Synchronized the implemented prototype into the Documents/GitHub workspace. | The modular source from `Desktop/수업자료/2-1.5/Somewhere` was copied into `Documents/somewhere`, with a pre-sync backup stored under `.sync-backups/`. The live browser server should now be restarted from `Documents/somewhere` when reviewing GitHub-bound work. |
| 2026-07-09 | Updated the agent harness for the modular prototype structure. | The review pipeline now includes split CSS/JS files and runs the regression tests plus the UX contract checker against the full `prototype/` source set. |
...
| 2026-07-08 | Add a simulated map-informed compass layer. | The compass now separates route bearing from user heading, so the needle feels like it is responding to an external direction signal without adding maps, GPS, or real APIs. | This tests the feel of map-linked direction, not actual navigation accuracy. |
| 2026-07-08 | Remove route-variable chips from the active compass surface. | The lower-center chips competed with the distance readout and made the compass feel crowded, especially at arrival. The active state now keeps the compass to bearing needle, cardinal marks, and distance only. | Route variables remain a later hypothesis rather than a v0.1 screen element. |
...
- Compass route bearing is simulated; route signals, traffic-light cues, stair cues, map app integration, GPS, and real route parsing are not shown in the active v0.1 surface.
...
- Real GPS
- Map-informed bearing bridge that feeds only direction and safety cues into the compass, without turning the UI into a map screen
- Real route-variable taxonomy for crossings, stairs, slopes, and signal waits, reintroduced only if it does not crowd the compass
- Vibration feedback
