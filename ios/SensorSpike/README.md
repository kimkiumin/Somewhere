...
3. Set Product Name to `SensorSpike`, Organization Identifier to `com.somewhere`, Bundle Identifier to `com.somewhere.sensorspike`, Interface to `SwiftUI`, Language to `Swift`, and minimum deployment to iOS 18.
...
5. Add `SensorSpike/SensorSpikeApp.swift`, `SensorSpike/LocationHeadingModel.swift`, and `SensorSpike/ContentView.swift` to the `SensorSpike` target. Add `SensorSpikeTests/LocationHeadingModelTests.swift` to the `SensorSpikeTests` target.
6. In the app target's **Info** settings, add `NSLocationWhenInUseUsageDescription` with this exact value: `Somewhere uses location and heading only for this supervised sensor feasibility test.`
7. Confirm **Signing & Capabilities** has no Background Modes capability. This spike must not request background location.
